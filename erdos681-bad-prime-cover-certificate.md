...
Then every odd `k` with `3<=k<=K` automatically satisfies the fourth-root size condition
...
If `p-1+k` is composite and has no prime divisor at most `k^2`, then its least prime factor is greater than `k^2`; together with the fourth-root size condition this is exactly a residual witness. Hence absence of a witness forces every composite endpoint to have a prime divisor `q<=k^2`.

Conversely, if every endpoint is either prime or has a prime divisor at most `k^2`, then no composite endpoint can have least prime factor greater than `k^2`, so no shift in the window is a witness. ∎
...
Equivalently, a prime divisor already present in the shift offset is unavailable as a killing factor.
...
For a fixed prime `q`, divisibility of the endpoint is exactly
...
Then a `K`-bad prime produces a covering statement
...
is the set of prime exceptions.
...
## Why this matters for the parent close

An almost-all-primes theorem estimates how often such covering certificates occur. The parent asks for more: show that **no such certificate exists for any sufficiently large prime**, or construct infinitely many primes carrying one.

That is now the frozen deterministic object for the universalization phase of the #681 close mission.

A terminal positive proof must eventually show that for every sufficiently large prime `p`, some admissible odd shift escapes both the coherent small-factor cover and the prime-exception set. A terminal negative proof must produce arbitrarily large primes for which the certificate covers the entire fourth-root window.

No density-one, finite-census, or conditional statement is a substitute for either terminal outcome.
